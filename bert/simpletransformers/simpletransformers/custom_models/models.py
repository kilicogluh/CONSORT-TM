...
class BertForMultiLabelSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config, pos_weight=None, n_top_sections=0):
        super(BertForMultiLabelSequenceClassification, self).__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.n_top_sections = n_top_sections
        self.classifier = nn.Linear(config.hidden_size + self.n_top_sections, self.config.num_labels)
        self.pos_weight = pos_weight

        self.init_weights()

    def forward(
        self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, labels=None, inputs_embeds=None
    ):
        outputs = self.bert(
            input_ids,
        ...
        if self.n_top_sections > 0:
            pooled_output = torch.cat((pooled_output, inputs_embeds.float()), dim=-1)            
