...
class ClassificationModel:
    ...
    def train_model(
        ...
        **kwargs,
        ...
        if "text" in train_df.columns and "labels" in train_df.columns:
            train_examples = [
                InputExample(i, text, None, label, input_emb)
                for i, (text, label, input_emb) in enumerate(zip(train_df["text"], train_df["labels"], train_df["top_section"]))
            ]
    ...
    def load_and_cache_examples(
        ...
        all_input_embs = torch.tensor([f.input_emb for f in features], dtype=torch.long)

        if output_mode == "classification":
            all_label_ids = torch.tensor([f.label_id for f in features], dtype=torch.long)
        elif output_mode == "regression":
            all_label_ids = torch.tensor([f.label_id for f in features], dtype=torch.float)
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_input_embs)
    ...
    def predict(self, to_predict, input_emb, multi_label=False):
        """
        ...
        if multi_label:
            eval_examples = [
                InputExample(i, text, None, [0 for i in range(self.num_labels)], input_emb[i]) for i, text in enumerate(to_predict)
            ]
        else:
            if isinstance(to_predict[0], list):
                eval_examples = [InputExample(i, text[0], text[1], 0, input_emb[i]) for i, text in enumerate(to_predict)]
            else:
                eval_examples = [InputExample(i, text, None, 0, input_emb[i]) for i, text in enumerate(to_predict)]
        if args["sliding_window"]:
    ...
    def _get_inputs_dict(self, batch):
        inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3], "inputs_embeds": batch[4]}
