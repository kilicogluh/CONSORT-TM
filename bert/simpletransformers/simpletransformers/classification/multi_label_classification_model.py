...
class MultiLabelClassificationModel(ClassificationModel):
    def __init__(
        ...
        **kwargs,
        ...
        if use_cuda:
            ...
            self.device = "cpu"
        self.n_top_sections = args["n_top_sections"]
        if self.pos_weight:
            self.model = model_class.from_pretrained(
                model_name, config=self.config, pos_weight=torch.Tensor(self.pos_weight).to(self.device), **kwargs
            )
        else:
            self.model = model_class.from_pretrained(model_name, config=self.config, n_top_sections = self.n_top_sections, **kwargs)
    ...
    def predict(self, to_predict, input_emb, multi_label=True):
        return super().predict(to_predict, input_emb, multi_label=multi_label)
